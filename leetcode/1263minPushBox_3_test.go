...
func minPushBox3(grid [][]byte) int {
	...
	for len(q) > 0 {
		...
		for k := 0; k < 4; k++ {
			...
			if sx == bi && sy == bj {
				...
				q = append([][]int{{f(sx, sy), f(bi, bj), d}}, q...)
			}
		}
	}
	return -1
